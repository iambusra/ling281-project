...
- Run 3 Turkish sentiment models on entries
...
# Sentiment models (Turkish only)
SENTIMENT_MODELS = {
    "savasy": "savasy/bert-base-turkish-sentiment-cased",
    "akoksal": "akoksal/Turkish-Sentiment-Model",
    "bomayesa": "bomayesa/TurkishSentimentBERT",
}
...
def add_title_category_flags(titles: pd.DataFrame) -> pd.DataFrame:
    ...
    def cat_flag(row, cat):
        if row["origin_folder"] != "gocmen_titles_data":
            return False
        return bool(REGEX_TITLE_CAT[cat].search(row["title"].lower()))
...
def map_label_to_sign(label_str: str) -> int:
    ...
    Works with Turkish and English-ish labels.
    """
...
def run_sentiment_model(texts, model_name, model_path, batch_size=BATCH_SIZE):
    """
    Run one Turkish sentiment model on a list of texts.
    Returns:
...
def run_all_sentiment(entries: pd.DataFrame, text_col: str = "clean_text") -> pd.DataFrame:
    """
    Run all three sentiment models on entries[text_col].
    Adds columns:
      sent_<model>_label, sent_<model>_score, sent_ensemble_score, sent_ensemble_sign
    """
    df = entries.copy()
...
def analyze_suri_vs_suriyeli(entries: pd.DataFrame):
    ...
    - Descriptive stats
    - Simple logistic-style regression using statsmodels
    """
    df = entries.copy()
    ...
    # prepare for logistic regression: neg_target ~ is_suri + year
    ...
    # add econ & crime flags as simple controls
    for key in ["econ", "crime"]:
        col = f"entry_{key}"
        if col not in df_simple.columns:
            df_simple[col] = False

    X = df_simple[["is_suri", "year_c", "entry_econ", "entry_crime"]].astype(float)
    X = sm.add_constant(X)
    ...
    try:
        ...
        out_csv2 = os.path.join(TABLE_DIR, "suri_suriyeli_logit_results.csv")
        results.to_csv(out_csv2, index=False)
        logger.info(f"Saved suri vs suriyeli logistic results to {out_csv2}")
    except Exception as e:
        logger.error(f"Error fitting suri vs suriyeli logit: {e}")
...
def export_extreme_sentiment_examples(entries: pd.DataFrame, top_k: int = 10):
    ...
    cols = [
        ...
        "date", "year",
        "sent_ensemble_score",
        "sent_savasy_score", "sent_akoksal_score", "sent_bomayesa_score",
        "clean_text",
    ]
...
def attach_title_flags_to_entries(entries: pd.DataFrame, titles: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    title_flags = titles[
        ["origin_folder", "title_file", "is_gocmen_title", "is_multeci_title", "is_siginmaci_title"]
    ].copy()
    df = entries.merge(
        title_flags,
...
def main():
    ...
    # for titles, use title_normalized if present, else title
